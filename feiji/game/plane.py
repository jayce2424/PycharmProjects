...
class Plane (pygame.sprite.Sprite):#继承精灵的对象
    """
    飞机的基类
    """
    #飞机的图片
    ...
    def shoot(self):
        ...
        self.bullets.add(bullet)  #加到这个精灵组
...
class OurPlane(Plane):
    ...
    def update(self,war):
        ...
        if rest:
            ...
            war.status=war.OVER
            #2.敌方飞机清除
            war.enemies.empty()#remove all sprites
    ...
    def move_up(self):
        """向上移动，超出范围之后，拉回来"""
        super().move_up()   #调用父类的方法
        if self.rect.top<0:
            self.rect.top=0
    def move_down(self):
        """向下移动，超出范围之后，拉回来"""
        super().move_down()   #调用父类的方法
        if self.rect.top>=self.height-self.plane_h:
            self.rect.top=self.height-self.plane_h
    def move_left(self):
        """向左移动，超出范围之后，拉回来"""
        super().move_left()   #调用父类的方法
        if self.rect.left<0:
            self.rect.left=0
    def move_right(self):
        """向左移动，超出范围之后，拉回来"""
        super().move_right()   #调用父类的方法
...
class SmallEnemyPlane(Plane):
    """敌方的小型飞机"""
    # 飞机的图片
    #plane_images = [constants.SMALL_ENEMY_PLANE_IMG] #基类里通过循环控制，及时一个也要写成数组
    plane_images = constants.SMALL_ENEMY_PLANE_IMG_LIST #基类里通过循环控制，及时一个也要写成数组
    # 飞机爆炸的图片
